...
/**
 * Mengkonversi video ke format yang kompatibel dengan WhatsApp menggunakan FFmpeg.
 * @param {string} inputFile - Path file input.
 * @param {string} outputDir - Direktori output.
 * @returns {Promise<string>} Path file output yang sudah dikonversi.
 */
async function convertVideoForWhatsApp(inputFile, outputDir) {
  return new Promise((resolve, reject) => {
    const outputFile = path.join(outputDir, `whatsapp_${Date.now()}.mp4`);

    // Parameter FFmpeg untuk konversi yang kompatibel dengan WhatsApp
    const ffmpegArgs = [
      "-i",
      inputFile,
      "-c:v",
      ...
      "aac", // Codec audio AAC
      "-preset",
      "fast", // Preset encoding cepat
      "-crf",
      "23", // Constant Rate Factor untuk kualitas yang baik
      "-movflags",
      "+faststart", // Optimasi untuk streaming
      "-pix_fmt",
      "yuv420p", // Pixel format yang kompatibel
      "-vf",
      "scale=trunc(iw/2)*2:trunc(ih/2)*2", // Pastikan resolusi genap
    ...
    ffmpeg.on("close", (code) => {
      if (code === 0 && fs.existsSync(outputFile)) {
        ...
        // Jika konversi gagal, kembalikan file asli
        resolve(inputFile);
      }
    });
    ...
    ffmpeg.on("error", (err) => {
      ...
      // Jika FFmpeg tidak tersedia, kembalikan file asli
      resolve(inputFile);
    });
  });
}
 ...
 * @param {boolean} convertForWhatsApp - Apakah perlu konversi untuk WhatsApp.
 */
async function sendMedia(sock, chatId, outputFile, options = {}) {
  const { isAudio, wantMp3, caption, convertForWhatsApp = true } = options;
  ...
  if (isAudio) {
    ...
    // Konversi video untuk kompatibilitas WhatsApp jika diminta
    if (convertForWhatsApp) {
      try {
        const tmpDir = path.dirname(outputFile);
        finalFile = await convertVideoForWhatsApp(outputFile, tmpDir);
        console.log(`Video dikonversi untuk WhatsApp: ${finalFile}`);
      } catch (error) {
        console.error("Gagal konversi video:", error);
        // Gunakan file asli jika konversi gagal
